package org.king2.aoppractice.exception;

import com.alibaba.fastjson.JSON;
import org.king2.aoppractice.config.SystemFilterConfiguration;
import org.king2.aoppractice.constant.SystemResultCodeEnum;
...
public class GlobalExceptionHandle {
	...
	public Object errorHandler(Exception exception) {
		...
		return SystemResultVo.build(SystemResultCodeEnum.ERROR.CODE,SystemResultCodeEnum.ERROR.MESS,null);
	}
	...
	public Object dataException(BindException exception) {
		pageExceptionLog(exception);
		// 返回数据校验错误信息
		return SystemResultVo.build(SystemResultCodeEnum.CHECK_ERROR.CODE, SystemResultCodeEnum.CHECK_ERROR.MESS, exception.getFieldError().getDefaultMessage());
	}
	...
	public Object constraintViolationExceptionHandler(ConstraintViolationException exception) {
		...
		// 返回错误信息
		return SystemResultVo.build(SystemResultCodeEnum.CHECK_ERROR.CODE, SystemResultCodeEnum.CHECK_ERROR.MESS, validResult.iterator().next().getMessage());
	}
	...
	public Object MethodArgumentNotValidException(MethodArgumentNotValidException exception) {
		pageExceptionLog(exception);
		// 返回数据校验错误信息
		return SystemResultVo.build(SystemResultCodeEnum.CHECK_ERROR.CODE, SystemResultCodeEnum.CHECK_ERROR.MESS, exception.getBindingResult().getFieldError().getDefaultMessage());
	}
